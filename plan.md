...
- **Phase 4: done.** Deviations: "left this cycle" is the server's `budget.totalRemaining` (the same number Check Pockets shows as Remaining, weekly pockets included via their whole-cycle allocation) rather than a client-side sum that skipped weekly pockets. An overspent cycle shows "Over budget this cycle" instead of an allowance. Fixed a pre-existing bug: without a week, the summary reported weekly pockets for the cycle's *first* week, so Monthly Story now re-requests the summary for the week containing today when there is a weekly pocket. Pocket Pulse hides pockets with no budget and no spending. The Month Feed header links to Review History ("View all") now that Today Feed and its link are gone.
- **Phase 5: done.** Deviations: one `#splitToggle` link whose text flips between "Split across pockets" and "Use one pocket" instead of two links; the old Single/Multi radios stay in the DOM (hidden) as form state. Split rows put the pocket picker on its own line with amount, Rest and remove below it, because four controls on one line cut pocket names to two letters at phone width. Rest uses a delegated listener (inline `onclick` doesn't run under JSDOM, so it couldn't be tested). Review History rows used to open a delete dialog on tap (no edit at all) and wrote notes, types, pocket names and emoji into the page unescaped; rows are now edit links and that text is escaped, same for emoji on Monthly Story. After a delete from the edit screen, the page returns to where the edit was opened from, like a saved edit.
- **Phase 6: done.** Deviations: a digits-only query (dots allowed, as in `35.000`) also matches the amount; Escape clears the search; the no-match message names the query. Fixed a pre-existing bug: Review History's summary card rewrote its count and total with dark text classes on the dark blue card, so they were close to unreadable; it now uses the same light classes as the server-rendered placeholder.

All six phases are done. Still out of scope (see the end of this file): moving the streak to the server. A small follow-up not in any phase: the split-row Add Pocket and remove buttons still use inline `onclick`, so they aren't covered by the JSDOM tests.
